...
export function AssignIssueModal({ row, open, pending, error, onClose, onSubmit, makerspaceId }: FormModalProps<AssignIssueValues> & { makerspaceId: number }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={row ? `Assign and issue request #${row.id}` : "Assign and issue"} footer={<FormFooter formId="assign-issue-form" pending={pending} submitLabel="Assign + issue" onCancel={onClose} />}>
      ...
      >
        {row?.assigned_box ? (
          <p className="text-xs text-muted">Box: <span className="font-medium text-ink">{row.assigned_box.label}</span></p>
        ) : null}
      ...
      <div className="flex min-w-0 flex-wrap gap-2">
        <div className="min-w-0 flex-1 rounded-md border border-line bg-surface px-3 py-2">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <span className={value ? "min-w-0 truncate font-medium text-ink" : "min-w-0 truncate text-muted"}>
              {value ? displayLabel || value : "No container selected"}
            </span>
