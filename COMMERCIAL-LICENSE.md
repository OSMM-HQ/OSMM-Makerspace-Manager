# OSMM — Commercial License

OSMM ("Open Systems Makerspace Manager") is available to the public under the
**PolyForm Noncommercial License 1.0.0** ([LICENSE.md](LICENSE.md)), which permits
noncommercial use only, plus the free grants in
[LICENSE-EXCEPTIONS.md](LICENSE-EXCEPTIONS.md) (notably: any makerspace may
self-host to run its own space, and nonprofits may use it internally).

**All commercial rights in OSMM are reserved to OSMM-HQ.**

You need a commercial license from OSMM-HQ if you want to do anything commercial
with OSMM that isn't already permitted above — for example:

- resell or redistribute OSMM as a paid product;
- host OSMM as a paid service (SaaS) for other organizations;
- bundle OSMM into a commercial product or offering;
- any other for-profit commercialization of the software itself.

Commercial licenses are granted at OSMM-HQ's discretion, on terms agreed in
writing. Using OSMM commercially without such a license is not permitted under the
public license and infringes OSMM-HQ's copyright.

## Contact

To request a commercial license, contact the **OSMM-HQ** organization via
<https://github.com/OSMM-HQ> with a short description of your intended use. A
dedicated commercial-licensing contact will be published here later.
